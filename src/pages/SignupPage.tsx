        ...
        >
          <div className="flex items-center justify-between mb-2.5">
            <button
              ...
              onClick={() => stepMonth(-1)}
              aria-label="이전 달"
              className="w-7 h-7 inline-flex items-center justify-center rounded-lg text-ink-soft hover:bg-bg-soft-2 transition-colors"
            >
              <ChevronLeftIcon className="w-4 h-4" />
            </button>
            <div className="text-sm font-bold text-ink tracking-[-0.01em]">
          ...
          <div className="grid grid-cols-7 gap-0.5 text-center">
            {DOW.map((d) => (
              <div
                key={d}
                className="text-[11px] font-semibold text-ink-mute py-1.5 tracking-[-0.01em]"
              >
                {d}
              </div>
            ))}
